...
_base = os.getenv("OPENAI_BASE_URL", "http://localhost:11434/v1").rstrip("/")
...
VALIDATOR_MODEL  = "llama3.1-8b-instruct"
PROCESSED_DIR    = PROJECT_ROOT / "data" / "processed"
DISTRAKTOR_DIR   = PROCESSED_DIR /  "data" / "processed" / "distraktor"
...
def is_substantive(content: str, min_chars: int = 50) -> bool:
...
def generate_thought_and_completion(
    ...
    t_instr = (
        "Buat 'thought_process' yang WAJIB menganalisis SETIAP dokumen yang diberikan satu per satu secara berurutan.\n"
        "Alur pikir (CoT) harus:\n"
        "1. Identifikasi inti pertanyaan.\n"
        "2. Evaluasi SETIAP dokumen (contoh: 'Dokumen 1: Tidak relevan karena membahas X. Dokumen 2: Relevan karena memuat Pasal Y tentang Z.').\n"
        "3. Pastikan jawaban murni berasal dari dokumen yang relevan tanpa tambahan fakta luar (halusinasi).\n"
        "4. Jika ternyata tidak ada satu pun dokumen yang relevan, simpulkan bahwa Anda harus menolak menjawab."
    )
